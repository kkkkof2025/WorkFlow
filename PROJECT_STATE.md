...
- **当前阶段**：最终全书复核阶段（第三阶段已全部完成）
- **当前批次**：批次 A-F 全部完成，30 个工作流全部生成
- **批次 F 状态**：✅ 已完成，构建通过
- **已完成章节数**：30 个工作流章节（01-30）+ 3 个黄金样章（含于 01-03）

---

## 权威编号来源（编号一致性基准）

- **权威编号来源**：文件名编号（如 `02-project-management.md` 即工作流 02）
- **为什么这样判断**：文件名编号是 MkDocs 路由的物理事实，且 `data/workflows.yaml` 与 mkdocs.yml 导航顺序均与之一致，三者互相印证。
- **本次修复原则**：最小破坏 —— 保持文件名不变，只修正正文 H1 标题编号、00 索引页、README 描述，使其全部对齐文件名编号；不重命名章节文件，不重排 01-30 文件编号。
- 详见 `docs/guide/numbering-audit.md`。
...
**第一条任务**：进行最终全书复核，参考 docs/guide/final-review.md，然后准备参赛提交材料。
